...
oil <-
  read_excel(
    "../Relevant Data/data_available_from_GRIIDC/GOMRI_R5x2860000002_supplementary_oil_data.xlsx",
    sheet = "data",
    na = "NA"
  )
...
oil <- oil %>%
  mutate(
    tot_naph = `C1-naphthalenes` + `C2-naphthalenes` + `C3-naphthalenes` + `C4-naphthalenes`,
    tot_phen = `C1-phenanthrenes` + `C2-phenanthrenes` + `C3-phenanthrenes` +
      `C4-phenanthrenes`,
    tot_dibenz = `C1-dibenzothiophenes` + `C2-dibenzothiophenes` +
...
oil$oil_added <-
  plyr::revalue(oil$oil_added, c("Y" = "Oiled", "N" = "No Oil Added")) %>%
  as.factor()
oil$orig_soil <-
  plyr::revalue(oil$orig_soil,
                c("Y" = "Prev-Oiled Inoculum", "N" = "Not Prev-Oiled Inoculum")) %>%
  as.factor()
oil$plant_trt <-
  plyr::revalue(oil$plant_trt, c("Y" = "Plant", "N" = "No Plant")) %>%
  as.factor()
...
clr.PAH <- oil %>%
  filter(
    oil_added == "Oiled",
    sampling_period %in% c("N16", "J18"),!(plantID == 60 &
                                             sampling_period == "J18")
  ) %>%
  select(tot_naph,
         tot_dibenz,
...
df.clr <- cbind(oil %>%
                  filter(
                    oil_added == "Oiled",
                    sampling_period %in% c("N16", "J18"),!(plantID ==
                                                             60 & sampling_period == "J18")
                  ),
                clr.PAH)
...
set.seed(1)
adonis(
  formula = clr.PAH ~ plant_trt * orig_soil,
  data = df.clr,
  permutations = perm,
  method = "euclidean"
)
