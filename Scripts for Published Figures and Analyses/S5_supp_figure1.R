...
shapefile.site <- st_read("../Relevant Data/shapefiles/porkchop_island/S5_Porkchop_Island-polygon.shp") 
#make data frame of annotations
annotations.site <- data.frame(x = c(-89.891, -89.887),
                          y = c(29.444, 29.439),
                          name = c("Heavily Oiled",
                                   "Reference"))
#plot
site.map <- ggplot() +
  geom_sf(data = shapefile.site,
          color = 'gray', 
          fill = '#6ed471', 
          alpha = 0.7,
          size = .2) +
  ...
  north(shapefile.site,
        ...
        anchor = c("x" = -89.889,
                   "y" = 29.437)) +
  ggsn::scalebar(data = shapefile.site,
           location = "bottomleft",
           dist = 0.25, 
           dist_unit = "km",
           transform = TRUE,
...
shapefile.state <- st_read("../data/shapefiles/states/states.shp") %>% 
...
#make data frame of annotations
annotations.state <- data.frame(x = c(-90.1, -90.1),
                          y = c(29.95, 29.42),
                          name = c("New Orleans",
                                   "Bay Jimmy"))
...
inset <- ggplot() +
  geom_sf(data = shapefile.state, 
          ...
          size = 0.2) +
  theme_void() +
  theme(panel.background = element_rect(fill = "#f7f8fa"),
        panel.border = element_rect(colour = "black",
                                    fill = NA)) +
  geom_text(data = annotations.state[1,],
             aes(x = x,
                 y = y,
                 label = name),
             hjust = 1,
             fontface = 2,
             size = annotation_size) +
  geom_point(data = annotations.state[1,],  #decided to only include New Orleans
             aes(x = x + 0.1,
                 y = y),
             size = 1/ggplot2::.pt)
...
p <- ggdraw(site.map) +
  draw_plot(inset,
            ...
            width = 0.3,
            height = 0.3) +
  geom_segment(data = arrow_points,
               aes(x = x1,
                   y = y1,
                   xend = x2,
                   yend = y2),
...
ggsave(filename = "S5_supp_fig1.pdf",
       path = "figures/", 
       width = 85,
       height = 85,
       units = "mm", 
...
ggsave(filename = "S5_supp_fig1.png",
       path = "figures/", 
       width = 85,
       height = 85,
       units = "mm", 
...
pdf_convert(pdf = "figures/S5_supp_fig1.pdf",
format = "tiff",
