...
p.ITS.roots <- cbind(df.meta[rownames(df.meta)!="S5_R60_J18",], A$points) %>%
  group_by(oil_added, orig_soil, sampling_period) %>% 
  dplyr::summarise(MDS1.mean = mean(MDS1),
            MDS2.mean = mean(MDS2)) %>% 
  ggplot(aes(x = MDS1.mean,
...
soil.legend <- get_legend(p.16S.soil +
...
top <- plot_grid(p.16S.soil + S5_theme + theme(legend.position = "none"),
                 p.ITS.soil + S5_theme + theme(legend.position = "none"), 
                 labels = c("Prokaryotes", "Fungi"), hjust = c(-0.38,-0.71))

p <- plot_grid(top, soil.legend, 
...
ggsave(filename = "S5_figure4.pdf",
       path = "figures/",
       width = 180,
       height = 85,
       units = "mm",
...
# #For draft and co-author review
ggsave(filename = "S5_figure4.png",
       path = "figures/",
       width = 180,
       height = 85,
       units = "mm",
...
pdf_convert(pdf = "figures/S5_figure4.pdf",
format = "tiff",
...
roots.legend <- get_legend(p.16S.roots +
                            theme(legend.position = "bottom", 
                                  legend.box="horizontal",
                                  legend.margin=margin(t = 1, r = 1, b = 1, l = 1),
                                  legend.title.align=0.5,
...
top <- plot_grid(p.16S.roots + S5_theme + theme(legend.position = "none"),
                 p.ITS.roots + S5_theme + theme(legend.position = "none"), 
                 labels = c("Prokaryotes", "Fungi"), hjust = c(-0.38,-0.71))

p <- plot_grid(top, roots.legend, 
          ncol = 1, rel_heights = c(1, 0.3))
...
pdf_convert(pdf = "figures/S5_figure5.pdf",
format = "tiff",
...
leaves.legend <- get_legend(p.16S.leaves +
                            theme(legend.position = "bottom", 
                                  legend.box="horizontal",
                                  legend.margin=margin(t = 1, r = 1, b = 1, l = 1),
                                  legend.title.align=0.5,
...
top <- plot_grid(p.16S.leaves + S5_theme + theme(legend.position = "none"),
                 p.ITS.leaves + S5_theme + theme(legend.position = "none"), 
                 labels = c("Prokaryotes", "Fungi"), hjust = c(-0.38,-0.71))

p <- plot_grid(top, leaves.legend, 
          ncol = 1, rel_heights = c(1, 0.3))
...
ggsave(filename = "S5_figure6.pdf",
       path = "figures/",
       width = 180,
       height = 85,
       units = "mm",
...
# #For draft and co-author review
ggsave(filename = "S5_figure6.png",
       path = "figures/",
       width = 180,
       height = 85,
       units = "mm",
...
pdf_convert(pdf = "figures/S5_figure6.pdf",
format = "tiff",
